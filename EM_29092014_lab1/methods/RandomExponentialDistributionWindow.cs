...
{
    public partial class RandomExponentialDistributionWindow : Form
    {
        MethodSelectWindow.SetRandom setRandom = null;

        public RandomExponentialDistributionWindow(MethodSelectWindow.SetRandom sr)
        {
            InitializeComponent();
            setRandom = sr;            
        }
        public RandomExponentialDistributionWindow()
        {
            InitializeComponent();
        }
        ...
        private void button1_Click(object sender, EventArgs e)//додати
        {
            try
            {
                ...
                RandomExponentialDistribution myRandom = new RandomExponentialDistribution(lambda, rand);
                setRandom(myRandom);
                Close();
            }
            catch (Exception exc)
