...
{
    class RandomBoxMullerSin : MyRandom
    {
        double n = 0;
        public RandomBoxMullerSin() : base()
        {
            n = 12;
        }
        public RandomBoxMullerSin(int n)
            : base()
        {
            this.n = n;
        }
        ...
        public override double NextDouble()//0...1
        {
            ...
            double ksi = Math.Sqrt(-2 * Math.Log10(alpha1)) * Math.Sin(2 * Math.PI * alpha2);
            if (n > 0)
        ...
        {
            return "Метод Бокса Мюллера (Формула для sin) ( n = " + n + ") -500...500";
        }
    }
}
